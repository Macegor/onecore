...
Функция ПолучитьТаблицуСвойств(Объект, Аннотация = Неопределено) Экспорт
	...
	Если Аннотация <> Неопределено Тогда
		...
		НоваяТаблицаСвойств = ТаблицаСвойств.СкопироватьКолонки();
...
Функция АннотацииМетода(Объект, ИмяМетода) Экспорт
	
	СтруктураАннотаций = Новый Структура();
	ТаблицаМетодов = ПолучитьТаблицуМетодов(Объект);
	НайденныеСтроки = ТаблицаМетодов.НайтиСтроки(Новый Структура("Имя", ИмяМетода));

	Если НайденныеСтроки.Количество() Тогда	
		ТаблицаАннотации = НайденныеСтроки[0].Аннотации;
		СтруктураАннотаций = ТаблицаАннотацийВСтруктуру(ТаблицаАннотации);	
	КонецЕсли;

	Возврат СтруктураАннотаций;

КонецФункции

Функция АннотацииСвойства(Объект, ИмяСвойства) Экспорт
...
Функция ТаблицаАннотацийВСтруктуру(ТаблицаАннотации) Экспорт

	СтруктураАннотаций = Новый Структура();

	Для каждого Аннотация Из ТаблицаАннотации Цикл

		ПараметрыАннотации = Новый Структура();

		Для каждого ПараметрАннотации Из Аннотация.Параметры Цикл

			Если НЕ ЗначениеЗаполнено(ПараметрАннотации.Имя) Тогда
				ПараметрыАннотации = ПараметрАннотации.Значение;
				Прервать;
			КонецЕсли;

			ПараметрыАннотации.Вставить(ПараметрАннотации.Имя, ПараметрАннотации.Значение);
			
		КонецЦикла;

		СтруктураАннотаций.Вставить(Аннотация.Имя, ПараметрыАннотации);
