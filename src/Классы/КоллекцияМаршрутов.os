...
Процедура ПриСозданииОбъекта(ОбъектКомпонента = Неопределено, СвойстваМаршрутовПоУмолчанию = Неопределено)
	...
	Если ТипЗнч(СвойстваМаршрутовПоУмолчанию) = Тип("Соответствие") Тогда
...
Процедура СвойстваМаршрутовПоУмолчанию(Соответствие) Экспорт
	Коллекции.ДополнитьСоответствие(СвойстваПоУмолчанию, Соответствие);
КонецПроцедуры
...
Функция Добавить(Знач Адрес, Знач КлючОбъекта, Знач Статический = Ложь) Экспорт
    ...
    НовыйОбъект = Новый Маршрут(Адрес, КлючОбъекта);
