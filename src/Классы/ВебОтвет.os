...
Процедура ПриСозданииОбъекта(Знач ВебЗапрос)
	...
	Соединение = Запрос.ПолучитьСоединение();
...
Процедура УстановитьТипСодержимого(Тип) Экспорт
	...
	ТипКонтента = ОбслуживаниеWebСервера.ОпределитьТипСодержимого(Тип);
...
Процедура Отправить(Код = Неопределено) Экспорт
	...
	ОписаниеКодаСостояния = ОбслуживаниеWebСервера.ПолучитьОписаниеКодаHTTP(КодСостояния);
	...
	АдресЗапроса = Запрос.ПолучитьАдрес();
