...
Функция ОпределитьСеанс(Контекст) Экспорт
	...
	Если Сеанс = Неопределено Тогда
		Сеанс = СоздатьСеанс();
		УстановитьКуку(Контекст.Ответ, "session_id", Сеанс.ПолучитьИдентификатор(), Истина, Ложь, Сеанс.ПолучитьВремяЖизни());
	КонецЕсли;
...
Процедура УстановитьКуку(Знач Ответ, Знач Ключ, Знач Значение, Защищена = Истина, ТолькоЗапросы = Ложь, ВремяЖизни = Неопределено) Экспорт
	
	Ключ = Строка(Ключ);

	Значение = Строка(Значение);
	Значение = ?(ВремяЖизни = Неопределено, "0", Значение + "; Max-Age=" + Формат(ВремяЖизни, "ЧГ=0"));
