...
Процедура ОбработатьЗапрос(Контекст, СледующийОбработчик) Экспорт
	...
	Исключение
		...
		Если ЗапросОтБраузера Тогда
			...
			Шаблонизатор = Новый Шаблонизатор(Неопределено, Новый Структура("Данные",  КонтекстОшибки));
			СтраницаОшибки = Шаблонизатор.Сформировать("OneCore.Шаблоны.ШаблонОшибки");
