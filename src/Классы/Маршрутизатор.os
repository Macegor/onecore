...
Процедура СформироватьМаршруты()
	...
	Для Каждого Компонент Из Компоненты Цикл
		...
		Для Каждого Маршрут Из КоллекцияМаршрутов Цикл
			...
			Если ТипЗнч(НовыйМаршрут) = Тип("Маршрут") Тогда
				...
				Рефлексия.УстановитьСвойство(НовыйМаршрут, "ПользовательскиеОбработчики", Маршрут.ПользовательскиеОбработчики());
...
Процедура ПеренаправитьПоИмени(Знач Контекст, Знач ИмяМаршрута) Экспорт
	ПеренаправитьНаАдрес(Контекст, АдресМаршрутаПоИмени(ИмяМаршрута));
КонецПроцедуры

Процедура ПеренаправитьНаАдрес(Знач Контекст, Знач Адрес) Экспорт
	Контекст.АдресПеренаправления = Адрес;
КонецПроцедуры
...
Процедура ПеренаправитьВыполнение(Контекст) Экспорт
	...
	Если Контекст.Запрос.ЕстьФормыВТипеКонтента = Истина Тогда
		
		ИмяФормы = "";
		ИмяФормыЛатинское = "";
		ВходящиеПоля = Новый Соответствие();

		Для каждого ПолеФормы Из Контекст.Запрос.Форма Цикл
			
			Если ПолеФормы.Ключ = "formname" Тогда
				ИмяФормыЛатинское = Строка(ПолеФормы.Значение);
				ИмяФормы = Сериализация.ЗаменитьСимволы(ИмяФормыЛатинское, Ложь);
				Продолжить;
			КонецЕсли;

			ИмяПоля = Строка(ПолеФормы.Ключ);
			ВходящиеПоля.Вставить(ИмяПоля, ?(ПолеФормы.Значение.Количество(), ПолеФормы.Значение[0], Неопределено));

		КонецЦикла;

		ОбъектФормы = Контекст.Формы.НайтиПоЛатинскомуИмени(ИмяФормыЛатинское);

		Если ОбъектФормы = Неопределено Тогда
			ОбъектФормы = Контекст.Формы.НайтиПоИмени(ИмяФормы);
		КонецЕсли;		

		Если ОбъектФормы <> Неопределено Тогда

			СтруктураТекущейФормы = Новый Структура("Объект", ОбъектФормы);
			ПоляФормы = ОбъектФормы.Поля();

			Для каждого ПолеФормыВходящее Из ВходящиеПоля Цикл

				ПолеФормы = ПоляФормы.НайтиПоЛатинскомуИмени(ПолеФормыВходящее.Ключ);
				Значение = "";

				Если ПолеФормы = Неопределено Тогда
					ПолеФормы = ПоляФормы.НайтиПоИмени(Сериализация.ЗаменитьСимволы(ПолеФормыВходящее.Ключ, Ложь));
				КонецЕсли;
				...
				ЗначениеЗаполнено = ЗначениеЗаполнено(ПолеФормыВходящее.Значение);

				Если ПолеФормы.ТипЗначения() = Тип("Булево") Тогда
					Значение = ?(ЗначениеЗаполнено, Булево(ПолеФормыВходящее.Значение), Ложь);
				Иначе
					Значение = ПолеФормыВходящее.Значение;
				КонецЕсли;

				ПолеФормы.Значение(Значение);

				СтруктураТекущейФормы.Вставить(ПолеФормы.Имя(), ПолеФормы);

			КонецЦикла;

			Контекст.ТекущаяФорма = СтруктураТекущейФормы;
			
			Если Рефлексия.СвойствоСуществует(ИсполняемыйОбъект, "ТекущаяФорма") Тогда
				Рефлексия.УстановитьСвойство(ИсполняемыйОбъект, "ТекущаяФорма", Контекст.ТекущаяФорма);
			КонецЕсли;
	...
	ПользовательскийОбработчик = Неопределено;
	Маршрут.ПользовательскиеОбработчики().Свойство(ТипЗапроса, ПользовательскийОбработчик);
	ВызываемыйМетод = ?(ПользовательскийОбработчик = Неопределено, ТипЗапроса, ПользовательскийОбработчик);

	Если НЕ Рефлексия.МетодСуществует(ИсполняемыйОбъект, ВызываемыйМетод) Тогда
		Ответ.КодСостояния = 405;
		Возврат;
	КонецЕсли;

	АннотацииМетода = Рефлексия.АннотацииМетода(ИсполняемыйОбъект, ВызываемыйМетод);
	...
	Рефлексия.ВызватьМетод(ИсполняемыйОбъект, ВызываемыйМетод, МассивПараметровВызова);
