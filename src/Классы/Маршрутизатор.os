...
Процедура ПеренаправитьВыполнение(Запрос, Ответ) Экспорт
	...
	Попытка
		...
		Если Рефлексия.МетодСуществует(ИсполняемыйОбъект, ТипЗапроса) Тогда
			ВызватьМетодПерехватчиков(ОбъектыПерехватчиков, "ПередВыполнениемПредставления", Запрос, Ответ);
			Рефлексия.ВызватьМетод(ИсполняемыйОбъект, ТипЗапроса, МассивПараметровВызова);
			ВызватьМетодПерехватчиков(ОбъектыПерехватчиков, "ПослеВыполненияПредставления", Запрос, Ответ);
		Иначе
...
Процедура ВызватьМетодПерехватчиков(ОбъектыПерехватчиков, ИмяМетода, Запрос, Ответ)
	...
	Для Каждого ОбъектПерехватчика Из ОбъектыПерехватчиков Цикл
		...
		Если НЕ Ответ.ОтветОтправлен() Тогда
			Если Рефлексия.МетодСуществует(ОбъектПерехватчика, ИмяМетода) Тогда
				...
				Исключение
					Лог.Предупреждение(СтрШаблон("Метод ""%1"" объекта перехватчика ""%2"" не выполнен, по причине: %3", ИмяМетода, СТрока(ОбъектПерехватчика), ОписаниеОшибки()));
				КонецПопытки;				
			КонецЕсли;
		КонецЕсли;
