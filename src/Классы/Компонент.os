...
Процедура ПриСозданииОбъекта(ИмяКомпонента, ИмяКомпонентаЛатинское, Отказ = Ложь)
	...
	МенеджерОбъектов.ПодключитьСервисыКОбъекту(МодульКомпонента, ЭтотОбъект);
...
Процедура ПодключитьПредставленияКомпонента()
	...
	Для каждого Файл Из ФайлыПредставлений Цикл
		...
		МенеджерОбъектов.ЗарегистрироватьОбъект(СокрЛП(Файл.ПолноеИмя), КлючОбъекта, Отказ);
...
Функция ОбъектСоответствуетМодели(Объект)
	...
	Для каждого СтрокаСвойства Из ТаблицаСвойств Цикл
		...
		ЭтоКолонка = АннотацииСвойства.Свойство("Колонка", ПараметрыАннотации);
		ЭтоПодчиненнаяТаблица = ?(НЕ ЭтоКолонка, АннотацииСвойства.Свойство("ПодчиненнаяТаблица", ПараметрыАннотации), Ложь);
		ЭтоПодчиненнаяТаблица = Ложь; // Временная блокировка возможности использования подичненных таблиц
		
		Если НЕ ЭтоКолонка И НЕ ЭтоПодчиненнаяТаблица Тогда
			Возврат Ложь;
		КонецЕсли;
		...
		Если НЕ ПараметрыАннотации.Свойство("Тип") ИЛИ НЕ ПараметрыАннотации.Свойство("Имя") Тогда
			Возврат Ложь;
		КонецЕсли;
