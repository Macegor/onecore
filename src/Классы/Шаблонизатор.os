...
Функция Маршрут(Имя, Парам1 = Неопределено, Парам2 = Неопределено, Парам3 = Неопределено, 
	Парам4 = Неопределено, Парам5 = Неопределено, Парам6 = Неопределено)

	МенеджерОбъектов = Приложение.МенеджерОбъектов();
	Ключ = МенеджерОбъектов.ПолныйКлючОбъекта(Имя, ТипыОбъектовOnecore.Маршруты, ОбъектПредставления);
	
	Возврат Маршрутизатор.АдресМаршрутаПоИмени(Ключ, Парам1, Парам2, Парам3, Парам4, Парам5, Парам6);
...
Функция Форма(ИмяФормы, Обработчик = Неопределено, Классы = "")

	ШаблонАтрибутов = "<form name=""%1"" id=""%1"" action=""%2"" method=""POST"" class=""%4"">
	|<input type=""hidden"" name=""formname"" value=""%3""/>";
	Форма = ОбщийКонтекст.Формы.НайтиПоИмени(ИмяФормы);

	Если Форма = Неопределено Тогда
		ВызватьИсключение СтрШаблон("Форма не найдена: ""%1""", ИмяФормы);
	КонецЕсли;

	ПутьКОбработчику = ?(Обработчик = Неопределено, "/", Маршрутизатор.АдресМаршрутаПоИмени(Обработчик));
	ИмяФормыЛатиница = Сериализация.ЗаменитьСимволы(ИмяФормы);

	Возврат СтрШаблон(ШаблонАтрибутов, ИмяФормыЛатиница, ПутьКОбработчику, ИмяФормыЛатиница, Классы);
...
Функция ПолеФормы(ПутьКДанным, Классы = "")
	
	ШаблонАтрибутов = "<input type=""%4"" name=""%1"" id=""%2"" value=""%3"" class=""%5""%6>";
	ЧастиПутиКДанным = СтрРазделить(ПутьКДанным, ".", Ложь);

	Если ЧастиПутиКДанным.Количество() <> 2 Тогда
		ВызватьИсключение СтрШаблон("Не верное значение пути к данным: ""%1""", ПутьКДанным);
	КонецЕсли;

	ИмяФормы = ЧастиПутиКДанным[0];
	ИмяПоля = ЧастиПутиКДанным[1];
	...
	ПолеФормы = Форма.Поля().НайтиПоИмени(ИмяПоля);

	Если ПолеФормы = Неопределено Тогда
		ВызватьИсключение СтрШаблон("Поле ""%1"" формы ""%2"" не найдено", ИмяФормы, ИмяПоля);
	КонецЕсли;

	ДополнительныеАтрибуты = Новый Соответствие();
	ДополнительныеЭлементы = Новый Массив();
	ИмяПоляЛатиница = Сериализация.ЗаменитьСимволы(ИмяПоля);

	Если ПолеФормы.ВыборИзСписка() Тогда
		
		ИдентификаторСписка = ИмяПоляЛатиница + "_list";
		ШаблонСписка = "";
		ДополнительныеАтрибуты.Вставить("list", ИдентификаторСписка);
		СписокВыбора = ПолеФормы.СписокВыбора();

		ШаблонСписка = ШаблонСписка + СтрШаблон("<datalist id=""%1"">", ИдентификаторСписка) + Символы.ПС;

		Для каждого ЭлементСписка Из СписокВыбора Цикл
			ШаблонСписка = ШаблонСписка + СтрШаблон("	<option value=""%1"">%2</option>", ЭлементСписка.Значение, ЭлементСписка.Представление) + Символы.ПС;
		КонецЦикла;

		ШаблонСписка = ШаблонСписка + Символы.ПС + "</datalist>";
		ДополнительныеЭлементы.Добавить(ШаблонСписка);

	КонецЕсли;

	ДопАтрибутыСтрокой = "";

	Для каждого ДопАтрибут Из ДополнительныеАтрибуты Цикл
		ДопАтрибутыСтрокой = ДопАтрибутыСтрокой + СтрШаблон(" %1=""%2""", ДопАтрибут.Ключ, ДопАтрибут.Значение);
	КонецЦикла;

	СформулированнаяВерстка = СтрШаблон(ШаблонАтрибутов, ИмяПоляЛатиница, ИмяПоляЛатиница, ПолеФормы.Значение(), ПолеФормы.ТипHTML(), Классы, ДопАтрибутыСтрокой);

	Для каждого ДопЭлемент Из ДополнительныеЭлементы Цикл
		СформулированнаяВерстка = СформулированнаяВерстка + Символы.ПС + ДопЭлемент;
	КонецЦикла;

	Возврат СформулированнаяВерстка;
...
Функция ТекстШаблонаПоПути(Знач КлючШаблона)
	...
	МенеджерОбъектов = Приложение.МенеджерОбъектов();
	Ключ = МенеджерОбъектов.ПолныйКлючОбъекта(КлючШаблона, ТипыОбъектовOnecore.Шаблоны, ОбъектПредставления);
...
Функция ПолучитьТипПоляВвода(Значение) Экспорт
	
	ТипЗначения = ТипЗнч(Значение);
	
	Если ТипЗначения = Тип("Число") Тогда
		Если Значение >= 0 И Значение <= 100 Тогда
			Возврат "range";
		Иначе
			Возврат "number";
		КонецЕсли;
	...
	ИначеЕсли ТипЗначения = Тип("Дата") Тогда
		Если Час(Значение) = 0 И Минута(Значение) = 0 И Секунда(Значение) = 0 Тогда
			Возврат "date";
		ИначеЕсли День(Значение) = 1 И Месяц(Значение) = 1 Тогда
			Возврат "time";
		Иначе
			Возврат "datetime-local";
		КонецЕсли;
	ИначеЕсли ТипЗначения = Тип("Строка") Тогда
		Если СтрНачинаетсяС(Значение, "http") Тогда
			Возврат "url";
		ИначеЕсли СтрНачинаетсяС(Значение, "tel:") Тогда
			Возврат "tel";
		ИначеЕсли СтрНачинаетсяС(Значение, "mailto:") 
			ИЛИ СтрНайти(Значение, "@") > 0 Тогда
			Возврат "email";
		ИначеЕсли СтрНачинаетсяС(Значение, "#") Тогда
			Возврат "color";
		Иначе
			Возврат "text";
		КонецЕсли;
