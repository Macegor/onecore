...
Функция Сформировать(Контекст = Неопределено) Экспорт

	ОбщийКонтекст = Контекст;
	...
	Если ЗначениеЗаполнено(РасширяемыйШаблон) Тогда
		...
		ТекстРасширяемогоШаблона = Шаблонизатор.Сформировать(Контекст);
...
Функция Вложить(КлючШаблона, Контекст = Неопределено)
	...
	Возврат ВложенныйШаблон.Сформировать(ПередаваемыйКонтекст);
