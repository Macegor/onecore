...
Функция Добавить(Знач Имя, Знач Заголовок, Знач Тип, Знач Вид, Знач ИмяЛатинское = Неопределено) Экспорт

    Если НайтиПоИмени(Имя) <> Неопределено Тогда
		ВызватьИсключение СтрШаблон("Поле с именем ""%1"" уже существует в форме", Имя);
	КонецЕсли;

    ПолеФормы = Новый ПолеФормы(Имя, Заголовок, Тип, Вид, ИмяЛатинское);
...
Функция НайтиПоЛатинскомуИмени(Знач Имя) Экспорт
    ...
    Для каждого ПолеФормы Из МассивКоллекции Цикл
		Если ПолеФормы.ИмяЛатинское() = Имя Тогда
			Возврат ПолеФормы;
		КонецЕсли;
	КонецЦикла;
