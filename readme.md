...
Для формирования шаблона используется встроенный класс "Шаблонизатор", в него передаётся представление и контекст, далее вызывается функция "Сформировать" с передачей в неё пути к шаблону, она вернет уже сформированный шаблон и его можно будет отправлять в теле ответа, например:
